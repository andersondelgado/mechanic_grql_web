...
import TarjetaRevisionModal from "../components/modales/TarjetaRevisionModal";

export default function TarjetasRevision() {
    ...
    return (
      <div className="flex h-96 items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-500 font-medium">Cargando tarjetas de revisión...</p>
        </div>
      </div>
    );
  }
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("¿Deseas eliminar esta tarjeta de revisión?")) {
      await remove(id);
    }
  };
    ...
    <div className="space-y-6 max-w-7xl mx-auto p-4 lg:p-8">
      ...
      <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h1 className="text-2xl font-bold text-secondary leading-tight">Tarjetas de Revisión</h1>
            <p className="text-sm text-gray-500 mt-0.5">Control de checklist e inspecciones internas y externas • <span className="font-semibold text-primary">{meta?.total ?? filteredData.length} en total</span></p>
          </div>
        </div>
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Buscar por elemento o tipo..."
              value={searchTerm}
          ...
          >
            <i className="fas fa-plus"></i> Nueva Tarjeta
          </button>
        </div>
      </div>
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/70 text-gray-500 font-bold uppercase text-[11px] tracking-wider">
                ...
                <th className="py-4 px-6 text-left">Elemento Revisado</th>
                <th className="py-4 px-6 text-center">Conforme (Sí)</th>
                <th className="py-4 px-6 text-center">Falla (No)</th>
                <th className="py-4 px-6 text-left">Observaciones</th>
              ...
              {filteredData.length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-8 text-center text-gray-400">
                    No se encontraron tarjetas de revisión
                  </td>
                </tr>
              ) : (
                filteredData.map((row: any) => (
                  <tr key={row.id || row.card_id} className="hover:bg-gray-50/70 transition-colors">
                    <td className="py-4 px-6 text-secondary font-bold">
                      <span className="px-2.5 py-1 bg-blue-50 text-primary rounded-lg text-xs font-semibold">
                        {row.inspection_type || 'REVISION'}
                      </span>
                    </td>
                    <td className="py-4 px-6 text-gray-700 font-medium">
                      {row.inspection_date ? new Date(row.inspection_date).toLocaleDateString('es-ES') : '—'}
                    </td>
                    ...
                    <td className="py-4 px-6 text-center">
                      <div className="flex items-center justify-center gap-1.5">
                        ...
                        <button
                          onClick={() => handleDelete(row.id || row.card_id)}
      ...
      <TarjetaRevisionModal
        isOpen={isModalOpen}
        onClose={() => setIsModalOpen(false)}
        onSuccess={refetch}
        tarjeta={selectedItem}
      />
    </div>
  );
}
