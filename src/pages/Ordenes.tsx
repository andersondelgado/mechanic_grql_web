...
import OrdenModal from "../components/modales/OrdenModal";

export default function Ordenes() {
  const [page, setPage] = useState(1);
  const { data, meta, loading, error, remove, refetch } = useGrqlList<any[]>("GestionTallerProd_body_shop_orders", {
  ...
  const handleOpenEdit = (orden: any) => {
    ...
    <div className="space-y-6 max-w-7xl mx-auto p-4 lg:p-8">
      ...
      <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h1 className="text-2xl font-bold text-secondary leading-tight">Órdenes de Trabajo</h1>
            <p className="text-sm text-gray-500 mt-0.5">Gestión de reparaciones y servicios • <span className="font-semibold text-primary">{meta?.total ?? filteredData.length} en total</span></p>
          </div>
        </div>
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Buscar por cliente, placa o # orden..."
              value={searchTerm}
            ...
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/70 text-gray-500 font-bold uppercase text-[11px] tracking-wider">
                  <th className="py-4 px-6 text-left"># Orden</th>
                  ...
                  <th className="py-4 px-6 text-right">Costo Estimado</th>
                ...
                {filteredData.length === 0 ? (
                  ...
                  filteredData.map((item: any) => {
                    ...
                    const entryDate = item.entry_date
                      ...
                      : "-";
                    const costVal = item.total_cost ?? item.cost ?? item.total ?? 0;
                    ...
                    return (
                      <tr key={item.id} className="table-row-hover hover:bg-blue-50/5 transition-colors">
                        <td className="py-4 px-6 font-bold text-primary font-mono text-xs">
                          {item.order_number || `OT-${String(item.id).substring(0, 6)}`}
                        </td>
