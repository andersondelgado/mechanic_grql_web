...
import ProveedorModal from "../components/modales/ProveedorModal";

export default function Proveedores() {
  const [page, setPage] = useState(1);
  const { data, meta, loading, error, remove, refetch } = useGrqlList<any[]>("GestionTallerProd_suppliers", {
    ...
    <div className="space-y-6 max-w-7xl mx-auto p-4 lg:p-8">
      ...
      <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h1 className="text-2xl font-bold text-secondary leading-tight">Proveedores</h1>
            <p className="text-sm text-gray-500 mt-0.5">Catálogo de proveedores del sistema • <span className="font-semibold text-primary">{meta?.totalElement ?? filteredData.length} en total</span></p>
          </div>
        </div>
            ...
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/70 text-gray-500 font-bold uppercase text-[11px] tracking-wider">
                  ...
                  <th className="py-4 px-6 text-left">Contacto</th>
                  <th className="py-4 px-6 text-left">Teléfono</th>
                  <th className="py-4 px-6 text-left">Email</th>
                  <th className="py-4 px-6 text-left">Distribuye</th>
                ...
                {filteredData.length === 0 ? (
                  ...
                  filteredData.map((item: any) => (
                    <tr key={item.id} className="table-row-hover hover:bg-blue-50/5 transition-colors">
                      ...
                      <td className="py-4 px-6 font-semibold text-secondary">{item.supplier_name}</td>
                      <td className="py-4 px-6 text-gray-600">{item.contact_person || '-'}</td>
                      <td className="py-4 px-6 text-gray-600">{item.cell_phone || '-'}</td>
                      <td className="py-4 px-6 text-gray-500">{item.email || '-'}</td>
                      <td className="py-4 px-6 text-gray-500 truncate max-w-[200px]">{item.distributes_parts || '-'}</td>
