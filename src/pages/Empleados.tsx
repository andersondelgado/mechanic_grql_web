...
import EmpleadoModal from "../components/modales/EmpleadoModal";

export default function Empleados() {
  const [page, setPage] = useState(1);
  const { data, meta, loading, error, remove, refetch } = useGrqlList<any[]>("GestionTallerProd_employees", {
    ...
    <div className="space-y-6 max-w-7xl mx-auto p-4 lg:p-8">
      ...
      <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h1 className="text-2xl font-bold text-secondary leading-tight">Personal y Empleados</h1>
            <p className="text-sm text-gray-500 mt-0.5">Registro de mecánicos y personal • <span className="font-semibold text-primary">{meta?.totalElement ?? filteredData.length} en total</span></p>
          </div>
        </div>
            ...
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/70 text-gray-500 font-bold uppercase text-[11px] tracking-wider">
                  ...
                  <th className="py-4 px-6 text-left">Cargo</th>
                  <th className="py-4 px-6 text-left">Salario</th>
                ...
                {filteredData.length === 0 ? (
                  ...
                  filteredData.map((item: any) => (
                    <tr key={item.id} className="table-row-hover hover:bg-blue-50/5 transition-colors">
                      ...
                      <td className="py-4 px-6 font-semibold text-secondary">{item.employee_name}</td>
                      <td className="py-4 px-6 text-gray-600">{item.tax_id}</td>
                      <td className="py-4 px-6 text-gray-600">{item.phone}</td>
                      <td className="py-4 px-6 text-gray-500">{item.position || '-'}</td>
                      <td className="py-4 px-6 font-semibold text-secondary">
                        {item.salary ? `$${Number(item.salary).toFixed(2)}` : '-'}
                      </td>
                      <td className="py-4 px-6">
                        <span className={`px-2.5 py-1 text-xs font-semibold rounded-full ${
                          item.is_mechanic ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'
                        }`}>
                          {item.is_mechanic ? 'Mecánico' : 'Otro'}
                        </span>
                      </td>
