...
import RepuestoModal from "../components/modales/RepuestoModal";

export default function Repuestos() {
  const [page, setPage] = useState(1);
  const { data, meta, loading, error, remove, refetch } = useGrqlList<any[]>("GestionTallerProd_parts_catalog", {
  ...
  const handleOpenEdit = (repuesto: any) => {
            ...
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/70 text-gray-500 font-bold uppercase text-[11px] tracking-wider">
                  ...
                  <th className="py-4 px-6 text-left">Código Parte</th>
                  ...
                  <th className="py-4 px-6 text-left">Precio Venta</th>
                ...
                {filteredData.length === 0 ? (
                  ...
                  filteredData.map((item: any) => (
                    <tr key={item.id} className="table-row-hover hover:bg-blue-50/5 transition-colors">
                      ...
                      <td className="py-4 px-6 font-bold text-primary font-mono">{item.part_number}</td>
                      <td className="py-4 px-6 font-semibold text-secondary">{item.description}</td>
