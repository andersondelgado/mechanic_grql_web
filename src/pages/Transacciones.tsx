...
import TransaccionModal from "../components/modales/TransaccionModal";

export default function Transacciones() {
  const [page, setPage] = useState(1);
  const { data, meta, loading, error, remove, refetch } = useGrqlList<any[]>("GestionTallerProd_financial_transactions", {
    ...
    return (
      <div className="flex h-96 items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-500 font-medium">Cargando movimientos financieros...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6 max-w-7xl mx-auto p-4 lg:p-8">
      ...
      <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h1 className="text-2xl font-bold text-secondary leading-tight">Ingresos & Gastos</h1>
            <p className="text-sm text-gray-500 mt-0.5">Control de movimientos de caja, operaciones y egresos del taller • <span className="font-semibold text-primary">{meta?.total ?? filteredData.length} en total</span></p>
          </div>
        </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredData.length === 0 ? (
                ...
                filteredData.map((row: any) => {
                  const isExpense = (row.total_expenses && row.total_expenses > 0) || row.type === 'Gasto';
                  const amount = Number(row.total_amount || row.total_expenses || 0);

                  return (
                    <tr key={row.id || row.transaction_id} className="hover:bg-gray-50/70 transition-colors">
                      ...
                      <td className="py-4 px-6 font-bold text-secondary">
                        {row.description || 'Movimiento Financiero'}
                      </td>
                      ...
                      <td className={`py-4 px-6 text-right font-bold ${isExpense ? 'text-red-600' : 'text-green-600'}`}>
                      ...
                      <td className="py-4 px-6 text-center">
                        <div className="flex items-center justify-center gap-1.5">
                          ...
                          <button
                            onClick={() => handleDelete(row.id || row.transaction_id)}
