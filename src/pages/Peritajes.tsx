      ...
      <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">Análisis iniciales y soporte multimedia • <span className="font-semibold text-primary">{meta?.totalElement ?? filteredData.length} en total</span></p>
          </div>
        </div>
            ...
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/70 text-gray-500 font-bold uppercase text-[11px] tracking-wider">
                  ...
                  <th className="py-4 px-6 text-left">Tipo Inspección</th>
                  <th className="py-4 px-6 text-left">Fecha</th>
                  <th className="py-4 px-6 text-left">Costo Estimado</th>
                ...
                {filteredData.length === 0 ? (
                  ...
                  filteredData.map((item: any) => (
                    <tr key={item.id} className="table-row-hover hover:bg-blue-50/5 transition-colors">
                      ...
                      <td className="py-4 px-6 text-gray-600 font-semibold">{item.inspection_type}</td>
                      <td className="py-4 px-6 text-gray-500">{item.inspection_date}</td>
                      <td className="py-4 px-6 font-semibold text-secondary">
                        {item.cost_estimate ? `$${Number(item.cost_estimate).toFixed(2)}` : '-'}
                      </td>
                      <td className="py-4 px-6">
                        <span className={`px-2.5 py-1 text-xs font-bold rounded-full uppercase ${
                          item.status === 'analizado' || item.status === 'completado' ? 'bg-green-100 text-green-800' :
                          item.status === 'grabando' ? 'bg-blue-100 text-blue-800' :
                          item.status === 'pendiente' ? 'bg-amber-100 text-amber-800' : 'bg-gray-100 text-gray-800'
                        }`}>
