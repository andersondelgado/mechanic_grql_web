...
import ContratoTrabajoModal from "../components/modales/ContratoTrabajoModal";

export default function ContratosTrabajo() {
    ...
    return (
      <div className="flex h-96 items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-500 font-medium">Cargando contratos laborales...</p>
        </div>
      </div>
    );
  }
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("¿Deseas anular o eliminar este contrato?")) {
      await remove(id);
    }
  };
    ...
    <div className="space-y-6 max-w-7xl mx-auto p-4 lg:p-8">
      ...
      <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h1 className="text-2xl font-bold text-secondary leading-tight">Contratos de Trabajo</h1>
            <p className="text-sm text-gray-500 mt-0.5">Control de contratos, acuerdos laborales y personal por comisión • <span className="font-semibold text-primary">{meta?.total ?? filteredData.length} en total</span></p>
          </div>
        </div>
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/70 text-gray-500 font-bold uppercase text-[11px] tracking-wider">
                ...
                <th className="py-4 px-6 text-left">Motivo / Tipo</th>
                <th className="py-4 px-6 text-left">Destino / Sede</th>
                <th className="py-4 px-6 text-left">Notas / Términos</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredData.length === 0 ? (
                ...
                filteredData.map((row: any) => (
                  <tr key={row.id || row.contract_id} className="hover:bg-gray-50/70 transition-colors">
                    <td className="py-4 px-6 text-gray-700 font-medium">
                      {row.issue_date ? new Date(row.issue_date).toLocaleDateString('es-ES') : '—'}
                    </td>
                    <td className="py-4 px-6 font-bold text-secondary">
                      {row.reason || 'Contrato Laboral'}
                    </td>
                    <td className="py-4 px-6 text-gray-600">
                      {row.destination || 'Sede Principal'}
                    </td>
                    ...
                    <td className="py-4 px-6 text-center">
                      <div className="flex items-center justify-center gap-1.5">
                        ...
                        <button
                          onClick={() => handleDelete(row.id || row.contract_id)}
