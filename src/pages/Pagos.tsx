...
import PagoModal from "../components/modales/PagoModal";

export default function Pagos() {
  const [page, setPage] = useState(1);
  const { data, meta, loading, error, remove, refetch } = useGrqlList<any[]>("GestionTallerProd_payments_and_transactions", {
    ...
    <div className="space-y-6 max-w-7xl mx-auto p-4 lg:p-8">
      ...
      <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h1 className="text-2xl font-bold text-secondary leading-tight">Pagos y Transacciones</h1>
            <p className="text-sm text-gray-500 mt-0.5">Control de ingresos y egresos • <span className="font-semibold text-primary">{meta?.totalElement ?? filteredData.length} en total</span></p>
          </div>
        </div>
                    ...
                    <tr key={item.id} className="table-row-hover hover:bg-blue-50/5 transition-colors">
                      ...
                      <td className="py-4 px-6">
                        <span className={`px-2.5 py-1 text-xs font-bold rounded-full uppercase ${
                          item.type === 'ingreso' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                        }`}>
                          {item.type}
                        </span>
                      </td>
                      <td className="py-4 px-6 text-gray-500">{item.category}</td>
                      <td className={`py-4 px-6 font-bold ${item.type === 'ingreso' ? 'text-green-600' : 'text-red-500'}`}>
                        {item.amount ? `$${Number(item.amount).toFixed(2)}` : '-'}
                      </td>
                      <td className="py-4 px-6 text-gray-500 truncate max-w-[200px]">{item.description || '-'}</td>
