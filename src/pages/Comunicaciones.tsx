...
import ComunicacionModal from "../components/modales/ComunicacionModal";

export default function Comunicaciones() {
  const [page, setPage] = useState(1);
  const { data, meta, loading, error, remove, refetch } = useGrqlList<any[]>("GestionTallerProd_communications", {
    ...
    return (
      <div className="flex h-96 items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-500 font-medium">Cargando comunicaciones internas...</p>
        </div>
      </div>
    );
  }
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("¿Deseas eliminar esta comunicación?")) {
      await remove(id);
    }
  };
    ...
    <div className="space-y-6 max-w-7xl mx-auto p-4 lg:p-8">
      ...
      <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h1 className="text-2xl font-bold text-secondary leading-tight">Comunicaciones Internas</h1>
            <p className="text-sm text-gray-500 mt-0.5">Memorandos, circulares y notificaciones al personal del taller • <span className="font-semibold text-primary">{meta?.total ?? filteredData.length} en total</span></p>
          </div>
        </div>
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          ...
          >
            <i className="fas fa-plus"></i> Nueva Comunicación
          </button>
        </div>
      </div>
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/70 text-gray-500 font-bold uppercase text-[11px] tracking-wider">
                ...
                <th className="py-4 px-6 text-left">Asunto</th>
                <th className="py-4 px-6 text-left">Contenido</th>
              ...
              {filteredData.length === 0 ? (
                <tr>
                  <td colSpan={5} className="py-8 text-center text-gray-400">
                    No se encontraron comunicaciones internas
                  </td>
                </tr>
              ) : (
                filteredData.map((row: any) => (
                  <tr key={row.id || row.communication_id} className="hover:bg-gray-50/70 transition-colors">
                    <td className="py-4 px-6 text-gray-700 font-medium">
                      {row.communication_date ? new Date(row.communication_date).toLocaleDateString('es-ES') : '—'}
                    </td>
                    <td className="py-4 px-6 font-semibold">
                      <span className="px-2.5 py-1 bg-purple-50 text-purple-700 font-bold text-xs rounded-lg">
                        {row.communication_type || 'MEMORANDUM'}
                      </span>
                    </td>
                    ...
                    <td className="py-4 px-6 text-center">
                      <div className="flex items-center justify-center gap-1.5">
                        ...
                        <button
                          onClick={() => handleDelete(row.id || row.communication_id)}
