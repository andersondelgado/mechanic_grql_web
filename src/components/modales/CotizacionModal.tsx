...
export default function CotizacionModal({ isOpen, onClose, onSuccess, cotizacion }: CotizacionModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (cotizacion?.id) {
        await updateEntity("GestionTallerProd_quotes", cotizacion.id, formData);
      } else {
        await createEntity("GestionTallerProd_quotes", formData);
      }
      onSuccess();
      onClose();
    } catch (err: any) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div>
                ...
                <input 
                  type="date" 
                  required
                  value={formData.quote_date ? String(formData.quote_date).split('T')[0] : ""} 
              ...
              <div>
                ...
                <input 
                  type="date" 
                  value={formData.valid_until ? String(formData.valid_until).split('T')[0] : ""} 
